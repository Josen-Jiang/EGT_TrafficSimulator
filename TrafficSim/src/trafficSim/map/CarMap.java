...
import trafficSim.car.context.TrafficStateCodifier;
...
/**
 * CarMap - Meta information management (in documentation referred to as
 * 'intersection').
 * 
...
public class CarMap extends TrafficMatrix {
	...
	// private ArrayList<TrafficLight> semList = new ArrayList<TrafficLight>();
	...
	private int NUM_TICKS_WITHOUT_MOVING = (Integer)p.getValue("max_ticks_without_moving");
	 ...
	 * @param normLayer
	 */
	public CarMap(Context<TrafficElement> context, Grid<TrafficElement> map, int carNumber) {
	 ...
	 */
	public void generateCars(int carNumber) {
		Car car;
		for (short i = 1; i <= carNumber; i++) {
			car = new Car(i, true);
			allCars.add(car);
	...
	private void moveCars() {
		...
		for (Car car : travelingCars) {
			...
			if(car.getTicksWithoutMovement() >= NUM_TICKS_WITHOUT_MOVING && car.getNextAction() != CarAction.Go){
				this.addCarToRemove(car);
			}else if(car.getNextAction().equals(CarAction.Stop)){
				car.setTicksWithoutMovement(1);
			}else{
				car.setTicksWithoutMovement(0);
			}
		...
		for (Car car : this.carsToRemove) {
			if(car.getTicksWithoutMovement() == 0){
				car.setSuccessfulJourneys(car.getSuccessfulJourneys() + 1);
			}
			remove(car);
		}

		List<Car> collidedCars = new ArrayList<Car>();
...
//					{
//						Car c = (Car)element;
//						c.setCollisions(c.getCollisions() + 1);
//					}
//				}

				removeCollided(elements);
				context.add(col);
				map.moveTo(col, x, y);
	...
	public void emitCars() {
		...
		double densityPercentage = TrafficConfiguration.SIM_CARS_DENSITY;
		int maxCarCells = 74;
		
		// Emit cars every N steps
		if (Main.getTick() == (long) (this.lastCarEmitTick + (long) TrafficConfiguration.SIM_NEW_CARS_FREQUENCY)) {
			//System.out.println("  - Emitting new cars...");
			
			
			float maxDensity = (float) (densityPercentage * maxCarCells);
			int carsToEmit = 0;
			
			if (maxDensity >= this.getNumCars()){
				carsToEmit = 2;
			}
			//int carsToEmit = Math.min(TrafficConfiguration.SIM_NUM_CARS_TO_EMIT, 4);
			CarPosition cp = null;

			this.lastCarEmitTick = Main.getTick();
	...
	public void broadcastAddNorm(Norm norm, Car c) {
		c.setGroup(norm.getId());
		c.getReasoner().addNorm(norm);
	}
	 ...
	 * @param norms
	 */
	public void broadcastRemoveNorm(Norm norm, Car c) {
			c.getReasoner().removeNorm(norm);
	}
	...
	private void add(CarPosition pos) {
		...
		car.init(pos);
		car.setJourneys(car.getJourneys() + 1);
		add(car);
	}
	 ...
	 * @param elements
	 */
	private void removeCollided(Iterable<TrafficElement> elements) {
	...
	private void remove(TrafficElement element) {
		...
		if (element instanceof Car) {
			//Remove from traveling list and add to available cars.
	 ...
	 * @return
	 */
	public boolean isFree(GridPoint p) {
		int count = 0;
